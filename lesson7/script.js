...
const game = {
    ...
    start() {
        ...
        window.addEventListener('keydown', game.startMoving);
    ...
    pause() {
        ...
        if (game.getGameStatus() === GAME_STATUS_STARTED) {
            ...
            window.removeEventListener('keydown', game.startMoving);
            pauseButton.innerHTML = "продолжить";
        } else if (game.getGameStatus() === GAME_STATUS_PAUSED) {
            game.setGameStatus(GAME_STATUS_STARTED);
            window.addEventListener('keydown', game.startMoving);
     ...
     * @param {keyboardevent} event Событие нажатия на клавишу
     * 
     */
    startMoving(event) {
        game.stopMoving();
        game.intervalId = window.setInterval(game.move, game.repeat, event);
    },
    ...
    move(event) {
        ...
        */
        ...
        for (let part of snake.parts) {
            if ((part.top === nextPosition.top) && (part.left === nextPosition.left)) {
                game.stop();
                return;
            }
        }
        ...
        if (foundFood !== -1) {
            ...
            /* перерендериваем еду */
            food.render();
            /* Ищем объект со счетом и увеличиваем счет */
            let scoreObj = document.getElementById("score-value");
            let score = Number(scoreObj.innerHTML);
            score++;
    ...
    setGameStatus(status) {
        ...
        // От Кирилла - сделал красивее, у меня функция
        // getGameStatus используется для паузы\продолжения
        // ну и здесь тоже
        element.classList.remove(game.getGameStatus());
...
const snake = {
    ...
    init() {
        ...
        */
         ...
         *
         * От Кирилла - поменял порядок, первый элемент - голова
         */
        this.parts = [
            { top: 0, left: 2 },
            { top: 0, left: 1 },
    ...
    setDirection(direction) {
        ...
        /* От Кирилла - переделал вот так, кода не стало сильно меньше, но вроде стало более читаемо */
